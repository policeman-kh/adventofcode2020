...
public class Main {
    ...
    private boolean isBecomeOccupied(int x, int y, char[][] matrix) {
        return (!exist(x - 1, y - 1) || matrix[x - 1][y - 1] != occupied)
               && (!exist(x, y - 1) || matrix[x][y - 1] != occupied)
               && (!exist(x + 1, y - 1) || matrix[x + 1][y - 1] != occupied)
               && (!exist(x + 1, y) || matrix[x + 1][y] != occupied)
               && (!exist(x + 1, y + 1) || matrix[x + 1][y + 1] != occupied)
               && (!exist(x, y + 1) || matrix[x][y + 1] != occupied)
               && (!exist(x - 1, y + 1) || matrix[x - 1][y + 1] != occupied)
               && (!exist(x - 1, y) || matrix[x - 1][y] != occupied)
    ...
    private boolean isBecomeEmpty(int x, int y, char[][] matrix) {
        int count = 0;
        if (exist(x - 1, y - 1) && matrix[x - 1][y - 1] == occupied) {
            count++;
        }
        if (exist(x, y - 1) && matrix[x][y - 1] == occupied) {
            count++;
        }
        if (exist(x + 1, y - 1) && matrix[x + 1][y - 1] == occupied) {
            count++;
        }
        if (exist(x + 1, y) && matrix[x + 1][y] == occupied) {
            count++;
        }
        if (exist(x + 1, y + 1) && matrix[x + 1][y + 1] == occupied) {
            count++;
        ...
        return count >= 4 && matrix[x][y] == occupied;
    }
